...
#load background
bkg = pygame.image.load('assets/img/background/background.png').convert_alpha()
...
true_button = Button((SCREEN_WIDTH / 2) - 175 , 380, true_img)
false_button = Button((SCREEN_WIDTH / 2) - 175, 500, false_img)
...
def Question_2():
    ...
    if fishing_net_button.draw():
        time.sleep(.1)
        point_count()
        increment()
    elif fishing_rod_button.draw():
        increment()
